...
?>
     ...
     */
    public function manageTicket($tickets, $currentPage, $totalPages, $totalTickets, $order)
    {
    ?>
        <div class="inline-flex rounded-md shadow-xs">
            <a href="<?php echo ($order === "dateASC") ? 'index.php?module=ticket&action=manageTicket&order=dateDESC' : 'index.php?module=ticket&action=manageTicket&order=dateASC'; ?>" aria-current="page" class="px-4 py-2 text-sm font-medium text-blue-700 bg-white border border-gray-200 rounded-s-lg hover:bg-gray-100 focus:z-10 focus:ring-2 focus:ring-blue-700 focus:text-blue-700 dark:bg-gray-800 dark:border-gray-700 dark:text-white dark:hover:text-white dark:hover:bg-gray-700 dark:focus:ring-blue-500 dark:focus:text-white">
                Creation Date <?php echo ($order === "dateASC") ? '^' : 'v'; ?>
            </a>
            <a href="<?php echo ($order === "prioASC") ? 'index.php?module=ticket&action=manageTicket&order=prioDESC' : 'index.php?module=ticket&action=manageTicket&order=prioASC'; ?>" class="px-4 py-2 text-sm font-medium text-gray-900 bg-white border-t border-b border-gray-200 hover:bg-gray-100 hover:text-blue-700 focus:z-10 focus:ring-2 focus:ring-blue-700 focus:text-blue-700 dark:bg-gray-800 dark:border-gray-700 dark:text-white dark:hover:text-white dark:hover:bg-gray-700 dark:focus:ring-blue-500 dark:focus:text-white">
                Priority <?php echo ($order === "prioASC") ? '^' : 'v'; ?>
            </a>
            <a href="<?php echo ($order === "statusASC") ? 'index.php?module=ticket&action=manageTicket&order=statusDESC' : 'index.php?module=ticket&action=manageTicket&order=statusASC'; ?>" class="px-4 py-2 text-sm font-medium text-gray-900 bg-white border border-gray-200 rounded-e-lg hover:bg-gray-100 hover:text-blue-700 focus:z-10 focus:ring-2 focus:ring-blue-700 focus:text-blue-700 dark:bg-gray-800 dark:border-gray-700 dark:text-white dark:hover:text-white dark:hover:bg-gray-700 dark:focus:ring-blue-500 dark:focus:text-white">
                Status <?php echo ($order === "statusASC") ? '^' : 'v'; ?>
            </a>
        </div>
        ...
        <div class="relative overflow-x-auto shadow-md sm:rounded-lg">
            ...
            <nav class="flex items-center justify-between pt-4" aria-label="Table navigation">
                ...
                <ul class="inline-flex -space-x-px text-sm">
                    <li>
                        <a href="?module=ticket&action=manageTicket&order=<?= $order ?>&page=<?= max(1, $currentPage - 1) ?>" class="flex items-center justify-center px-3 h-8 ms-0 leading-tight text-gray-500 bg-white border border-gray-300 rounded-s-lg hover:bg-gray-100 hover:text-gray-700 dark:bg-gray-800 dark:border-gray-700 dark:text-gray-400 dark:hover:bg-gray-700 dark:hover:text-white">Previous</a>
                    </li>
                    <?php for ($i = 1; $i <= $totalPages; $i++) : ?>
                        <li>
                            <a href="?module=ticket&action=manageTicket&order=<?= $order ?>&page=<?= $i ?>" class="<?= $i === $currentPage ? 'bg-blue-500 text-white' : 'bg-white text-gray-500 hover:bg-gray-100 hover:text-gray-700 dark:bg-gray-800 dark:text-gray-400 dark:hover:bg-gray-700 dark:hover:text-white' ?> flex items-center justify-center px-3 h-8 leading-tight border border-gray-300 dark:border-gray-700"><?= $i ?></a>
                        </li>
                    <?php endfor; ?>
                    <li>
                        <a href="?module=ticket&action=manageTicket&order=<?= $order ?>&page=<?= min($totalPages, $currentPage + 1) ?>" class="flex items-center justify-center px-3 h-8 leading-tight text-gray-500 bg-white border border-gray-300 rounded-e-lg hover:bg-gray-100 hover:text-gray-700 dark:bg-gray-800 dark:border-gray-700 dark:text-gray-400 dark:hover:bg-gray-700 dark:hover:text-white">Next</a>
                    </li>
                </ul>
            </nav>
    ...
    {
        $title = 'View a ticket | Ticket Pro +';
        $heading = 'View a ticket';

        $descriptionValue = htmlspecialchars($ticket['t_description']);
        $dueDateValue = htmlspecialchars($ticket['t_due_date']);
        if ($dueDateValue) {
            $dateObj = \DateTime::createFromFormat('Y-m-d H:i:s', $dueDateValue);
            if ($dateObj) {
                $dueDateValue = $dateObj->format('m/d/Y');
        ...
        $clientId = htmlspecialchars($ticket['c_id']);
        $projectId = htmlspecialchars($ticket['p_id']);
        $statusValue = htmlspecialchars($ticket['s_id']);
        $priorityValue = htmlspecialchars($ticket['pty_id']);
        $developerId = htmlspecialchars($ticket['u_id']);
